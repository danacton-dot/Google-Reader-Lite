...
function normalizeItem(it: any, isAtom=false) {
  if (isAtom) {
    const id = it.id || it.link?.['@_href'] || it.link?.[0]?.['@_href'] || it.title || Math.random().toString(36).slice(2)
    const link = typeof it.link === 'string' ? it.link : (it.link?.['@_href'] || it.link?.[0]?.['@_href'] || '')
    return {
      id,
    ...
    const content = typeof it['content:encoded'] === 'object' ? (it['content:encoded']?.['#text'] || '') : (it['content:encoded'] || '')
    return {
      id,
      title: it.title || '',
      link: it.link || '',
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (isRSS) {
      ...
      siteLink = typeof feed?.link === 'string' ? feed?.link : (feed?.link?.['@_href'] || feed?.link?.[0]?.['@_href'] || '')
      const entries = Array.isArray(feed?.entry) ? feed.entry : (feed?.entry ? [feed.entry] : [])
      items = entries.map((it: any) => normalizeItem(it, true))
    } else {
      return NextResponse.json({ error: 'Unsupported feed (expect RSS/Atom)' }, { status: 415 })
    }

    // sort newest first
    items.sort((a: any, b: any) => (new Date(b.published || 0).getTime()) - (new Date(a.published || 0).getTime()))
